...
class ApiRequestor {
  ...
  static async getFeatureServerStatus() {
    const result = await this.getJSON(
      CONNECTION_PROPERTIES.FeatureServer.LandingPage,
      {
        errorMsg: API_REQUESTOR.SERVER_ERROR,
      }
    );
  ...
  static async transactWFS(mode, feature) {
    ...
    switch (mode) {
      case "insert":
        transactionRequest = wfsSerializer.writeTransaction(
          [feature],
          null,
          null,
          defaultParam
    ...
    const wfsUpdateAsString =
      xmlSerializer.serializeToString(transactionRequest);
    const wfsRequest = await this.postJSON(
      CONNECTION_PROPERTIES.GEOSERVER.URL,
      {
        body: wfsUpdateAsString,
        errorMsg: CONNECTION_PROPERTIES.GEOSERVER.ERROR,
      }
    );
    const transactionResult = new WFS().readTransactionResponse(
      await wfsRequest.text()
    );
